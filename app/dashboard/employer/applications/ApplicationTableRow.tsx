...
export default function ApplicationTableRow({ app, isMobile }: { app: any; isMobile?: boolean }) {
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="font-bold text-slate-900 truncate">{app.seeker.name}</div>
                <div className="text-xs text-slate-400 truncate">{app.seeker.email}</div>
              </div>
            </div>
            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold shrink-0 ${statusColor}`}>
              {app.status}
            </span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <div>
              ...
              <div className="flex items-center gap-1 text-xs text-slate-400 mt-0.5">
                <Calendar className="h-3 w-3" /> {format(new Date(app.createdAt), 'MMM d, yyyy')}
              </div>
            </div>
